(function() {
    ...
    const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxuOY8WNXD384d5LgEi6eaxpNGMWh5zAMXQtzeUY5Ef838jG7uAVl2pDePHKfNZQ14S/exec";
    ...
    function fetchTrackerDataEmbro() {
        ...
        fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'get_tracker_embro', password: 'WASHD1' }),
            headers: { 'Content-Type': 'text/plain;charset=utf-8' }
        })
        .then(res => res.json())
        .then(data => {
    ...
    function executeGenerateSummaryEmbro() {
        ...
        fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({
                action: 'generate_summary_embro', password: 'WASHD1',
    ...
    async function downloadEmbroSummaryExcel() {
        ...
        try {
            const res = await fetch(SCRIPT_URL, {
                method: 'POST',
                body: JSON.stringify({
                    action: 'preview_generate_summary_embro', password: 'WASHD1',
                    buyer: selectedGenEmbroData.buyer, style: selectedGenEmbroData.style, color: selectedGenEmbroData.color,
                    supplier: document.getElementById('ge_supplier').value,
                    qty_order: document.getElementById('ge_qty_order').value,
                    shipment: document.getElementById('ge_shipment').value,
    ...
    function fetchTrackerDataForPreview() {
        ...
        fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'get_tracker', password: 'WASHD1' }),
            headers: { 'Content-Type': 'text/plain;charset=utf-8' }
        })
        .then(res => res.json())
        .then(data => {
    ...
    function fetchTodayData() {
        ...
        fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'get_today_data', password: 'WASHD1', tanggal }),
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            signal: controller.signal
        })
        .then(res => { clearTimeout(timeoutId); return res.text(); })
    ...
    function finalizeTutupShift() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(SCRIPT_URL, {
                    method: 'POST',
                    body: JSON.stringify({ action: 'finalize_shift', password: 'WASHD1', tanggal }),
                    headers: { 'Content-Type': 'text/plain;charset=utf-8' }
                })
                .then(res => res.json())
                .then(data => {
    ...
    function executeGenerateSummaryBuyer() {
        ...
        fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'generate_summary', password: 'WASHD1', buyer: selectedTrackerData.buyer, style: selectedTrackerData.style, color: selectedTrackerData.color, supplier, shipment, per_style: perStyle }),
            headers: { 'Content-Type': 'text/plain;charset=utf-8' }
        })
        .finally(() => {
            document.getElementById('loading').style.display = 'none';
